...
export default function App() {
  // Use 'isDarkMode' (boolean) and 'setIsDarkMode' (function)
  ...
  useEffect(() => {
    ...
    // Apply class to the main div based on state
  }, [isDarkMode]);

  return (
    <FirebaseProvider>
      {/* Apply base background/text color classes based on isDarkMode state */}
      <div
        id="main-app-container"
        className={`h-screen w-screen font-sans antialiased transition-colors duration-300 ${isDarkMode ? 'dark bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}
      >
        {/* Pass isDarkMode and setIsDarkMode as props */}
...
// Corrected the component signature to accept isDarkMode and setIsDarkMode
const CalendarAppContent = ({ isDarkMode, setIsDarkMode }) => {
  ...
  const handleMarkCompleted = async (taskId) => {
    ...
    try {
      const taskDocRef = doc(db, `artifacts/${appId}/users/${userId}/tasks`, taskId);
      await deleteDoc(taskDocRef); // Deleting from tasks collection
  ...
  return (
    <div className={`flex h-screen overflow-hidden font-sans antialiased transition-colors duration-300 ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      <aside className={`w-1/4 min-w-[280px] p-6 m-4 rounded-3xl backdrop-blur-2xl bg-white bg-opacity-10 dark:bg-gray-800 dark:bg-opacity-10 text-black dark:text-white flex flex-col transition-colors duration-300 border border-gray-200 dark:border-gray-700 shadow-xl`}>
        <div className="flex items-center mb-6 space-x-2">
          <img
            ...
            onError={(e) => { e.target.src = "https://placehold.co/40x40/50c878/fff?text=U"; }}
          />
        ...
        <div className="flex-shrink-0 flex items-center justify-between mt-6">
          ...
          <button
            onClick={() => setIsDarkMode(!isDarkMode)} // Correctly uses the prop
            className="p-2 rounded-full hover:bg-white hover:bg-opacity-10 transition-colors"
            title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"} // Correctly uses the prop
          >
            {/* Show Sun icon when in Dark Mode, Moon icon when in Light Mode */}
            {isDarkMode ? <Sun size={24} /> : <Moon size={24} />} {/* Correctly uses the prop */}
          </button>
          {/* Sign In/Out Buttons */}
